...
const assumeToBeFollower1 = () => {
  ...
  const partition1 = ring.follower.defaultPartitioner('key1')
  map.set(partition1, 'key1')

  // Do the same for other data
  // get the partition for this key , you can use another partitioner if needed.
  const partition2 = ring.follower.defaultPartitioner('key2')
  map.set(partition2, 'key2')
  const partition3 = ring.follower.defaultPartitioner('key3')
  map.set(partition3, 'key3')
  const partition4 = ring.follower.defaultPartitioner('key4')
  map.set(partition4, 'key4')
  const partition5 = ring.follower.defaultPartitioner('key5')
  ...
  const assignedPartitions = follower.partitions()
  ...
  assignedPartitions.forEach(p => {
    ...
    if (map.get(p)) { console.log(`Now I can work on object with this key ${map.get(p)} that belong to this partition ${p}`) }
  })
}
